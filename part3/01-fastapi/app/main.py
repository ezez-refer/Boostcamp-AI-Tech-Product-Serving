...
from app.model import MyEfficientNet, get_config, get_model, predict_from_image_byte
...
class Order(BaseModel):
    ...
    def add_product(self, product: Product):
        # add product는 Product를 인자로 받아서, 해당 id가 이미 존재하는지 체크 => 없다면 products 필드에 추가
...
def get_order_by_id(order_id: UUID) -> Optional[Order]:
    ...
    # 제너레이터
    # 익숙하지 않다면 iter, next 키워드로 검색
    # 제너레이터를 사용한 이유 : 메모리를 더 절약해서 사용할 수 있음
    # 이터레이터, 이터러블, 제너레이터 => 파이썬 면접에서 많이 나오는 소재. GIL
    # iter는 반복 가능한 객체에서 이터레이터를 반환
    # next는 이터레이터에서 값을 차례대로 꺼냄


@app.post("/order", description="주문을 요청합니다")
async def make_order(files: List[UploadFile] = File(...), model: MyEfficientNet = Depends(get_model), config: Dict[str, Any] = Depends(get_config)):
    # Depends : 의존성 주입
